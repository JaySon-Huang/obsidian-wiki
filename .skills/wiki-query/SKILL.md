...
2. Resolve a local checkout root: `.projects.<name>.source_cwd_hint` (a `~`-relative hint) if present, otherwise the page's `source_path` frontmatter. Expand `~` and confirm the directory actually exists before using it.
3. Legacy manifests may still have `source_cwd` (an absolute path) — read it for backward compatibility, but do not propagate it into your answer as the project's identity.

...
- **跨机搬迁**：旧 vault 根与新 vault 根、`$HOME` 都不匹配时无法词法剥离，故提供 `--from-root <旧 vault 根>`（可重复），显式剥离旧根；未指定时对共享前缀的绝对 key 打印 `HINT` 建议该 root，但仅当该候选通过校验（剥离后至少一条相对 key 的首段是当前 vault 顶层目录名）才打印可复制的命令，否则只提示需要真正的旧根；
